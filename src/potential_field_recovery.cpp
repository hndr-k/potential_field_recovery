...
namespace potential_field_recovery {
    ...
    PotentialFieldRecovery::PotentialFieldRecovery(): Recovery<Action>(), costmap_sub_(nullptr) {}
    ...
    void PotentialFieldRecovery::onConfigure() {
                    ...
                    if (!initialized_) {
                        ...
                        node->get_parameter("sim_granularity", sim_granularity_);
                         ...
                         nav2_util::declare_parameter_if_not_declared(
                            node,
                            "frequency", rclcpp::ParameterValue(20.0));
                        node->get_parameter("frequency", frequency_);

                        nav2_util::declare_parameter_if_not_declared(
                            node,
                            "min_dist", rclcpp::ParameterValue(0.4));
                        node->get_parameter("min_dist", min_dist_);


                        nav2_util::declare_parameter_if_not_declared(
                            node,
                            "max_rotational_vel", rclcpp::ParameterValue(0.2));
                        node->get_parameter("max_rotational_vel", max_rotational_vel_);

                        nav2_util::declare_parameter_if_not_declared(
                            node,
                            "max_trans_vel", rclcpp::ParameterValue(0.3));
    ...
    while(rclcpp::ok())
    {
       ...
       for(int posX = 0; posX < width; ++posX)  {
           for(int posY = 0; posY < height; ++posY) {
                ...
                costmap_sub_->getCostmap()->mapToWorld(posX, posY, cur_world_x, cur_world_y);
                if(costmap_sub_->getCostmap()->getCost(posX, posY) != nav2_costmap_2d::FREE_SPACE)  {
...
PLUGINLIB_EXPORT_CLASS(potential_field_recovery::PotentialFieldRecovery, nav2_core::Recovery)
